	...
	return a2a.AgentCard{
		...
		Skills: []a2a.AgentSkill{
			...
			},
			{
				ID:          "maze_heist",
				Name:        "Maze Heist",
				Description: "Explore a maze, coordinate with peers, and solve challenges in the maze heist game.",
				Tags:        []string{"game", "maze", "heist", "explore"},
			},
		},
...
func main() {
	...
	// Shared explorer ID for game tools. Set when the agent joins the game.
	explorerID := ""

	// a2aHandler feeds incoming A2A messages (from the A2A server or the game
	// relay inbox) into the agent loop and waits for the response.
	...
	// Set up context with Ctrl+C handling. Created early so the explorer
	// goroutine (started on join) gets the right context.
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt)
	defer stop()

	// Create the autonomous explorer. It handles deterministic movement in
	// Go code and only sends messages to the LLM when something interesting
	// is found (doors, jewel, broadcasts). This cuts LLM calls by ~85%.
	...
	for _, t := range []tools.Tool{
		...
		tools.Move{ExplorerID: &explorerID},
		tools.Look{ExplorerID: &explorerID},
		tools.UseAbility{ExplorerID: &explorerID},
		tools.SubmitKey{ExplorerID: &explorerID},
	} {
